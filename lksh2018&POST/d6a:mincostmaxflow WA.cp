...
struct edge{
    ...
    ll w;
    edge(){};
    edge(int u, int v, int c, ll w): u(u), v(v), c(c), x(0), f(0), w(w){};
};
...
vector<int> path;
void FB(int s, int t, int ste, int k) {
    path.clear();
    memset(d, -1, sizeof(d));
...
void mincirculation(int k) {
    for(int k = 1 << 17; k > 0; k >>= 1) {
