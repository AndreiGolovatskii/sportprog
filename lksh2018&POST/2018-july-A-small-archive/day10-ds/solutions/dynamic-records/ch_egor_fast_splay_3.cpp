...
static const int buf_size = 15 * 1000 * 1000;
...
inline void setParent(int child, int parent)
{
	if (child != -1)
	{
...
inline void updateChilds(int parent)
{
	if (parent != -1)
	{
...
inline void rotate(int child, int parent)
{
...
inline int lowerBound(int x, int key)
{
	if (x == -1) return -1;
	...
	while (true)
	{
		...
		{
			x = nodes[x].left; // return lowerBound(nodes[x].left, key);
		}
		else if (nodes[x].value < key && nodes[x].right != -1)
		{
			x = nodes[x].right; // return lowerBound(nodes[x].right, key);
		}
		else
		{
			return splay(x);
...
inline void split(int root, int &t1, int &t2, int key)
{
	if (root == -1)
	{
		t1 = t2 = -1;
