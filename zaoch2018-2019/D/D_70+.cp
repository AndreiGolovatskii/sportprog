...
const int MN = 5e5 + 123, MQ = 25e4 + 11, MQC = 5e4 + 11;
...
inline bool operator<(const que &a, const que &b) {
    return a.a / TMPDECSIZE == b.a / TMPDECSIZE ? a.b < b.b : a.a / TMPDECSIZE < b.a / TMPDECSIZE;
}
...
struct decq{
    ...
    inline void process(){
        if(allc < alls && allq[allc].a / TMPDECSIZE == id) {
            curr_mex.clear();
        }
        while(allc < alls && allq[allc].a / TMPDECSIZE == id) {
            auto i = allq[allc++];
...
int n;
int getTmpSize() {
    int r = (int)sqrt(n * 1.0 / alls * n) + 1;
    return min(DECSIZEMAX, max(r, DECSIZEMIN));
}
void sol(){
    TMPDECSIZE = getTmpSize();
    sort(allq, allq + alls);
    for(int i = 0; allc < alls && i * TMPDECSIZE < n; ++i) {
        queries[i].id = i;
        queries[i].r = queries[i].l = i * TMPDECSIZE;
        queries[i].process();
    }
...
int32_t main() {
    ...
    for(int i = 0; i < q; ++i) {
        ...
        if((ll)ch_que_sz * alls >= (ll)alls * getTmpSize()) {
            sol();
        }
    }
    sol();
